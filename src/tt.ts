...
function showExistingEvent(): void {
    ...
    function displayParticipants(): void {
        ...
        participants.forEach(participant => {
            ...
            const costs: CostDetails[] = participantCosts[participant] || []; // You can simplify or keep the check

            // Calculate total cost for the participant
            const totalCost: number = costs.reduce((sum, cost) => sum + cost.amount, 0);

            // Format the costs display
            const costsDisplay: string = costs.map(cost => `€${cost.amount.toFixed(2)} on ${cost.date} for ${cost.description}`).join(", ");

            // Display participant with total costs
            p.innerText = `${participant} - Total Costs: €${totalCost.toFixed(2)} | Costs: ${costsDisplay || "No costs added"}`;
...
function showExistingEvent(): void {
    ...
    function displayParticipants(): void {
        ...
        participants.forEach(participant => {
            ...
            // Format the costs display
            const costsDisplay: string = costs
                .map((cost, index) => {
                    return `€${cost.amount.toFixed(2)} on ${cost.date} for ${cost.description} 
                <button onclick="deleteCost('${participant}', ${index})">Delete</button>`;
                })
                .join("<br>"); // Using <br> to separate each cost

            // Display participant with total costs
            p.innerHTML = `${participant} - Total Costs: €${totalCost.toFixed(2)} <br> Costs: <br> ${costsDisplay || "No costs added"}`;
            participantsContainer.appendChild(p);
        });
    }
...
function showExistingEvent(): void {
        ...
        participants.forEach(participant => {
            ...
            if (costs.length > 0) {
                ...
                costs.forEach((cost, index) => {
                    ...
                    deleteButton.addEventListener("click", () => deleteCost(participant, index)); // Bind the delete action
    ...
    function deleteCost(participant: string, index: number): void {
        ...
        if (participantCosts[participant]) {
            ...
            // If the participant has no more costs, you might want to clean up
            if (participantCosts[participant].length === 0) {
                participantCosts[participant] = []; // Optionally remove the participant if no costs left
            }
